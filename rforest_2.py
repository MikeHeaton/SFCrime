...
processed_data_X = processed_data[['year','X','Y','day','hour','district']]
...
test_data_X = test_data[['year','X','Y','day','hour','district']]
submission = test_forest.predict(test_data_X)

types = processed_data['Category'].unique().tolist()
types.sort()
submission_formatted = pd.DataFrame(columns = ['Id']+types, index = range(0,len(submission)))
